...
class TestHelmert(unittest.TestCase):
    ...
    def test_fit_translation(self):
        helmert = HelmertTransform(translation=[1e-6, 2e-6, 3e-6], rotation=[0, 0, 0], 
                                   config={'degrees':True})
        ...
        for _i in range(10):
            helmert2.fit(vector, target, 
                         params={'translation':True, 'rotation':False, 'scale':False})
        self.assertTrue(np.all(helmert.get_params() - helmert2.get_params() < 1e-16))

    def test_fit_scale(self):
        helmert = HelmertTransform(translation=[0,0,0], rotation=[0, 0, 0], scale=5.2e-5,
                                    config={'degrees':True})
        vector = np.random.rand(100, 3)
        target = helmert.apply(vector)
        helmert2 = HelmertTransform()
        for _i in range(10):
            helmert2.fit(vector, target,
                        params={'translation':False, 'rotation':False, 'scale':True})
        self.assertTrue(np.all(helmert.get_params() - helmert2.get_params() < 1e-16))
        
    def test_fit_rot(self):
        helmert = HelmertTransform(translation=[0,0,0], rotation=[1/3600.0, 2/3600.0, 0], scale=0,
                                   config={'degrees':True})
        ...
        for _i in range(10):
            helmert2.fit(vector, target,
                         params={'translation':False, 'rotation':True, 'scale':False})
        self.assertTrue(np.all(helmert.get_params() - helmert2.get_params() < 1e-16))
        
    def test_fit_combined(self):
        helmert = HelmertTransform(translation=[1e-3,1e-4,2e-3], rotation=[1/3600.0, 2/3600.0, 4/3600.0], scale=1e-6,
                                    config={'degrees':True})
        vector = np.random.rand(100, 3)
        target = helmert.apply(vector)
        helmert2 = HelmertTransform()
