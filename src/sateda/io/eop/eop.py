...
class EOP:
    ...
    def interpolate(self, mjd: float, degree: int) -> None:
...
if __name__ == "__main__":
    ...
    for i, d in enumerate(date):
        # print(d)
        lagrange_data[i, :] = eop.interpolate(d, 3)
    ...
    for i in range(6):
        ...
        ax[i,0].plot(date, linear_data[:, i])
        ax[i,1].plot(date, lagrange_data[:, i] - linear_data[:, i])
    plt.show()