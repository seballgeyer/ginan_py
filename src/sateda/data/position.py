...
def xyz2blh(x, y, z):
    ...
    R, phi = np.hypot(xy_hypot[small_angle_indices], z[small_angle_indices]), np.arctan(
        z[small_angle_indices] / xy_hypot[small_angle_indices]
    )
...
class Position:
    ...
    def rotate_enu(self) -> None:
        ...
        for data in self.data:
            ...
            base.epoch = base.epoch[in_base]
            for k in base.data:
                base.data[k] = base.data[k][in_base]
            data.epoch = data.epoch[in_data]
            for k in data.data:
                data.data[k] = data.data[k][in_data]
